...
func (a auth) GetSessionInfo(ctx context.Context, session string) (models.SessionInfo, error) {
	const op = "auth.GetSessionInfo"

	info, err := a.sessionSrv.GetSessionInfo(ctx, session)
	if err != nil {
